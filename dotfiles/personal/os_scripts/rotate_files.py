...
def rotate_file(target_file_path, directory):
    ...
    if not os.path.isfile(target_file_path):
        print(f"The target file {target_file_path} does not exist. Will copy the first file in the directory.")
    ...
    files = sorted([f for f in os.listdir(directory) if os.path.isfile(os.path.join(directory, f)) and f != log_file_name])
